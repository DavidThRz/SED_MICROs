...
//-------DEFINES PARA EL SENSOR DE TEMPERATURA
#define pdte_temp 0.01
#define voltaje_25 0.75
...
volatile int puerta_abierta = 0;		//0->Puerta cerrada --- 1->Puerta abierta
...
float voltaje_sensor_temp = 0;		//Valor leido del sensor
...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  //---Boton 5: Puerta del garaje
	  	  if(antirebote(BOTON_5_PORT,BOTON_5_PIN, button_5, button_counter_5, counter_5) == 1){
	  		  if(puerta_abierta == 0){
	  			__HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_1, 100);
	  			puerta_abierta = 1;
	  		  }
	  		  else if(puerta_abierta == 1){
	  			__HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_1, 200);
	  			puerta_abierta = 0;
	  		  }
	  		  button_5 = flag_button;
	  	  }
	  	  button_counter_5 = flag_button_counter; counter_5 = flag_counter;


	  //CASOS:
	  	  //Caso 0: Modo manual
	  	  //Caso 1: Modo Automatico
	  	  //Caso 2: Modo Alarma
	  switch (estado){
	  case 0:
		  //Leds que informan de que estado actual
	  ...
	  //-----Lectura del sensor ded temperatura
	  voltaje_sensor_temp = ((float)adc2/255)*3.3;
	  temperatura = (voltaje_sensor_temp - voltaje_25)/(pdte_temp) + 25;
	  ...
	  //-----Asignacion de valores al buffer


	  buffer_envio[0] = temperatura;	//Temperatura
	  buffer_envio[1] = adc1;			//Luz
	  buffer_envio[2] = car_presence;	//Hay coche klk


	  if(HAL_GetTick()-counter_esp > 15000){
		 //ESP_Send_Multi("CPHVATCVOA26M2FS", 3 , buffer_envio);
		 counter_esp = HAL_GetTick();
	  }
